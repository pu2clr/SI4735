//  Version 4.2 18-12-2020 Add noise surpression during 1kHz frequency change in SSB.
//  Version 4.1 12-12-2020 Automatic AM Antenna tuning added as option
//  Version 4.0 15-10-2020 SI5351 added as option
//  Version 2.9 08-07-2020 Seekfreq not in  EEprom solved.
//  Version 2.8 28-06-2020 More BFO tuning bugs solved
//  Version 2.7 27-06-2020 Tuning bug solved;
//  Version 2.6 27-06-2020 BFO calculation added.
//  Version 2.5 15-06-2020 Improved BFO tuning ans some bug support
//  Version 2.4 01-06-2020 S-Meter calibrated. BFO changed to part of tuning. Tuning +- 10Hz
...
#define MIN_ELAPSED_AudMut_TIME      150   //  audio mute ssb 1khz freq. change
...
#define CORRECTION_FACTOR 78369 //SI5351 PROTO PRINT #2
//#define CORRECTION_FACTOR 26254 //SI5351  SI4732 #1
#define DAC1 25
...
uint16_t Xbut8  = 3 * Xbutsiz ; uint16_t Ybut8  =-4 * Ybutsiz;
uint16_t Xbut9  = 3 * Xbutsiz ; uint16_t Ybut9  =-3 * Ybutsiz;
uint16_t Xbut10 = 3 * Xbutsiz ; uint16_t Ybut10 =-2 * Ybutsiz;
uint16_t Xbut11 = 3 * Xbutsiz ; uint16_t Ybut11 =-1 * Ybutsiz;
#endif
...
Button bt[] = {         //                                                 
  ...
  { "MODE"  ,  9 , "AntTun", 6 , Xbut6 , Ybut6  }, //     |----|----|----|     
  { "BANDW" ,  5 , "AllTun", 7 , Xbut7 , Ybut7  }, //     |  9 | 10 | 11 |     
...
Band band[] = {
  {   "FM", FM_BAND_TYPE,  FM,  8750, 10800,  9890,10,   0,    0,   0,    0}, //  FM          0
  {   "LW", LW_BAND_TYPE,  AM,   100,   513,   198, 9,   0,    0,   0,    0}, //  LW          1
  {   "MW", MW_BAND_TYPE,  AM,   522,  1701,  1008, 9,   0,    0,   0,    0}, //  MW          2
  {"BACON", LW_BAND_TYPE,  AM,   280,   470,   284, 1,   0,    0,   0,    0}, // Ham          3
  { "630M", SW_BAND_TYPE, LSB,   472,   479,   475, 1,   0,    0,   0,    0}, // Ham  630M    4
  { "160M", SW_BAND_TYPE, LSB,  1800,  1910,  1899, 1,   0,    0,   0,    0}, // Ham  160M    5
  { "120M", SW_BAND_TYPE,  AM,  2300,  2495,  2400, 5,   0,    0,   0,    0}, //      120M    6
  {  "90M", SW_BAND_TYPE,  AM,  3200,  3400,  3300, 5,   0,    0,   0,    0}, //       90M    7
  {  "80M", SW_BAND_TYPE, LSB,  3500,  3800,  3700, 1, -30, 3630,-140, 3780}, // Ham   80M    8
  {  "75M", SW_BAND_TYPE,  AM,  3900,  4000,  3950, 5,   0,    0,   0,    0}, //       75M    9
  {  "60M", SW_BAND_TYPE, USB,  5330,  5410,  5375, 1,   0,    0,   0,    0}, // Ham   60M   10
  {  "49M", SW_BAND_TYPE,  AM,  5900,  6200,  6000, 5,   0,    0,   0,    0}, //       49M   11
  {  "40M", SW_BAND_TYPE, LSB,  7000,  7200,  7132, 1,-120, 7000,-120, 7200}, // Ham   40M   12
  {  "41M", SW_BAND_TYPE,  AM,  7200,  7450,  7210, 5,   0,    0,   0,    0}, //       41M   13
  {  "31M", SW_BAND_TYPE,  AM,  9400,  9900,  9600, 5,   0,    0,   0,    0}, //       31M   14
  {  "30M", SW_BAND_TYPE, USB, 10100, 10150, 10125, 1,   0,    0,   0,    0}, // Ham   30M   15
  {  "25M", SW_BAND_TYPE,  AM, 11600, 12100, 11700, 5,   0,    0,   0,    0}, //       25M   16
  {  "22M", SW_BAND_TYPE,  AM, 13570, 13870, 13700, 5,   0,    0,   0,    0}, //       22M   17
  {  "20M", SW_BAND_TYPE, USB, 14000, 14350, 14200, 1,   0,14000,   0,14350}, // Ham   20M   18
  {  "19M", SW_BAND_TYPE,  AM, 15100, 15830, 15700, 5,   0,    0,   0,    0}, //       19M   19
  {  "17M", SW_BAND_TYPE, USB, 18068, 18168, 18100, 1,   0,    0,   0,    0}, // Ham   17M   20
  {  "16M", SW_BAND_TYPE,  AM, 17480, 17900, 17600, 5,   0,    0,   0,    0}, //       16M   21
  {  "15M", SW_BAND_TYPE,  AM, 18900, 19020, 18950, 5,   0,    0,   0,    0}, //       15M   22
  {  "15M", SW_BAND_TYPE, USB, 21000, 21450, 21350, 1,   0,    0,   0,    0}, // Ham   15M   23
  {  "13M", SW_BAND_TYPE,  AM, 21450, 21850, 21500, 5,   0,    0,   0,    0}, //       13M   24
  {  "12M", SW_BAND_TYPE, USB, 24890, 24990, 24940, 1,   0,    0,   0,    0}, // Ham   12M   25
  {  "11M", SW_BAND_TYPE,  AM, 25670, 26100, 25800, 5,   0,    0,   0,    0}, //       11M   26
  {   "CB", SW_BAND_TYPE,  AM, 26200, 27990, 27200, 1,   0,    0,   0,    0}, // CB band     27
  {  "10M", SW_BAND_TYPE, USB, 28000, 30000, 28500, 1,   0,28000,   0,28700}, // Ham   10M   28
  {   "SW", SW_BAND_TYPE,  AM,  1730, 30000, 15500, 5,   0,    0,   0,    0}  // Whole SW    29
};
...
#define offsetEEPROM       0x20//   was 0x30
...
//=======================================================================================
  ...
  startfreq = 522;
  stopfreq = 1701;
  freqnum = ((stopfreq-startfreq)/9)+1;
  ...
  Serial.println("Version 4.2 18-12-2020");
  tft.println("SI4735  Radio");
  tft.setCursor(7, 70);
  tft.println(" Version 4.2");
  tft.setCursor(7, 95);
  tft.println(" 18-12-2020");
  tft.setTextColor(TFT_BLUE, TFT_BLACK);
  tft.setCursor(7, 120);
  tft.println("AM antenna tuner");
  tft.setCursor(7, 140);
  tft.println("SSB Noise mute");
  tft.setCursor(7, 170);
  delay(1000);
  ...
  for (int t = 0; t < sizeof(storage); t++) {
    Serial.print(t);
    Serial.print(" = ");
    Serial.println( EEPROM.read(offsetEEPROM + t));
  }

  #ifdef IhaveWindowAnt
  for (int t = 0 ; t < freqnum; t++) {
    Serial.print(t);
    Serial.print(" = ");
    Serial.print(startfreq+(t*9));
    Serial.print("KHz  ");
    Serial.println( EEPROM.read(offsetEEPROM + t + 75));
  }
  #endif
...
//=======================================================================================
  ...
  delay(10);
  for (unsigned int t = 0; t < freqnum; t++) {
    if (EEPROM.read(offsetEEPROM + t + 75) != SelfLearnAT[t]){
      EEPROM.write(offsetEEPROM + t + 75, SelfLearnAT[t]);
    } 
  }  
  EEPROM.commit();
...
//x= EEPROM.read(offsetEEPROM + t + 150);
...
//=======================================================================================
  ...
  //while (((pressed == false) and (encoderCount == 0) and (encoderButton == 0)) or (writingEeprom)) {  // wait loop  
  while (((pressed == false) and (encoderCount == 0) and (encoderButton == 0) and (encBut > 500)) or (writingEeprom)) {  // wait loop    
      ...
      for (int n = 0 ; n <= lastButton; n++) {
        if ((x > (Xbutst + (bt[bt[n].ButtonNum1].XButos))) and (x < Xbutst + (bt[bt[n].ButtonNum1].XButos) + Xbutsiz) and (y > Ybutst + (bt[bt[n].ButtonNum1].YButos)) and (y < Ybutst + (bt[bt[n].ButtonNum1].YButos) + Ybutsiz)) {
         ...
         if (n == SEEKDN) {
           ...
           //si4735.setSeekAmSNRThreshold(20);
           //si4735.setSeekFmSNRThreshold(20);
           if ((currentMode != LSB) and (currentMode != USB))   {
             if (currentMode != FM) {     // No FM
               if (band[bandIdx].bandType == MW_BAND_TYPE || band[bandIdx].bandType == LW_BAND_TYPE) {
                 si4735.setSeekAmSpacing(band[bandIdx].currentStep);     //9 KHz
...
//=======================================================================================
  if (encoderCount != 0)
  {
    ...
    if (mainpurp == 1)
    {
      ...
      if (currentMode == LSB || currentMode == USB) {
         if (encoderCount == 1) {
           currentBFO = currentBFO - freqstep;
           ...
           Serial.print("currentBFO in up encodercount = ");
           Serial.println(currentBFO);
         } else {
           currentBFO = currentBFO + freqstep;
           freqDec = freqDec - freqstep;
           if (freqDec < 0)  {
             freqDec = 1000-freqstep;
             si4735.frequencyDown();
             BFOlin();
             currentBFO= BFOcalq-freqDec;
           }
           Serial.print("currentBFO in down encodercount = ");
           Serial.println(currentBFO);
         }  
      }else  {     
        if (encoderCount == 1) {
...
//=======================================================================================
  ...
  if (encBut < 500) {
    ...
    delay(400);
      ...
      if (currentMode == LSB || currentMode == USB) {
        freqstepnr = freqstepnr + 1;
        if (freqstepnr == 3) freqstepnr =0;
        if (freqstepnr == 0)  {
          freqstep = 1000;
          freqDec = 0;
          BFOlin();
          currentBFO = BFOcalq;
          Serial.print("step 1khz bfo = ");
          Serial.println(currentBFO);
        }  
...
//=======================================================================================  
  if ((FirstLayer) or (ThirdLayer)) {
    ...
    #ifdef IhaveWindowAnt
    if ((band[bandIdx].bandType == MW_BAND_TYPE) or (band[bandIdx].bandType == LW_BAND_TYPE)){
      int q = ((currentFrequency - startfreq)/9);
      dacWrite(DAC1,EEPROM.read(offsetEEPROM + q + 75));
      delay(150);
    }
    #endif
...
void BFOlin() {
  BFOcalq = (((band[bandIdx].BFOf1-band[bandIdx].BFOf2)/(band[bandIdx].F1b-band[bandIdx].F2b))*si4735.getFrequency())+band[bandIdx].BFOf1-(band[bandIdx].F1b*((band[bandIdx].BFOf1-band[bandIdx].BFOf2)/(band[bandIdx].F1b-band[bandIdx].F2b)));
  Serial.print("FREQ + BFOlincalq = ");
  Serial.print(si4735.getFrequency());
  Serial.print(" + ");
  Serial.println(BFOcalq);
} 
...
//=======================================================================================  
  int RssiArray[260];
  int NPointArray[260];
  RSSIfact = 0.3; // RSSI delay 0,3 * 150 ms 
  int RI = 0;
  ...
  for (int n = 0 ; n <= 200; n++) {
    ...
    Serial.print(" RSSI  n= ");
    Serial.print(n);
    Serial.print("  Ant. Cap = ");
    Serial.println(si4735.getAntennaTuningCapacitor());
  }
  for (int n = 0 ; n <= 200; n++) {
      if (RssiArray [n] > RI) {
        RI  = RssiArray [n];
  ...
  dacWrite(DAC1,RIn);
  Serial.print("Max RSSI van ");
  Serial.print(RI); 
  Serial.print(" bij DA step : ");
  Serial.println(RIn);
}

//=======================================================================================
void SelfLearnAntTune() {
//=======================================================================================
  si4735.setFrequency(startfreq);
  for (int n = 0 ; n < freqnum ; n++) {
   FreqDispl();
   AntTune();
   SelfLearnAT[n]=RIn;
   Serial.print("Opgeslagen DACval : ");
   Serial.print(SelfLearnAT[n]);
   Serial.print(" voor freq.  : ");
   Serial.print(si4735.getFrequency());
   Serial.println(" KHz");
   si4735.frequencyUp();    
   band[bandIdx].currentFreq = si4735.getFrequency();
