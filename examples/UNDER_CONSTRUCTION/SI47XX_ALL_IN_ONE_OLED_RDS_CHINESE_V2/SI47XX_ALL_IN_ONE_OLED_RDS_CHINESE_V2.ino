...
const DCfont *digit1632Font  = FONT16X32DIGITS;
...
void showStatus()
{
  ...
  {
    oled.print("BW:");
    oled.print(bandwitdthSSB[bwIdxSSB]);
    oled.print("kHz");
    showBFO();
  }
  ...
  {
    oled.print("BW:");
    oled.print(bandwitdthAM[bwIdxAM]);
    oled.print("kHz");
  }
...
void showBFO()
{
  ...
  // Saving memory avoiding sprintf or String or similar (see convertToChar implementation)
  if (currentBFO > 0) {
    convertToChar(currentBFO, &bfo[1], 4);  
    bfo[0] = '+'; 
  }
  else {
    convertToChar((uint16_t)(~currentBFO + 1), &bfo[1], 4);
    bfo[0] = '-';
  }
