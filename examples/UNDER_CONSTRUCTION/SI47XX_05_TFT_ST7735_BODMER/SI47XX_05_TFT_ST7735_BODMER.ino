/*
  ...
  This sketch uses just one encoder with bush button as the user interface. 
...
// Overwrite default defined constants
...
// Buttons controllers
#define ENCODER_PUSH_BUTTON 16 // Pin A0/14
...
void clearFrequency() {
    ...
    tft.drawString("88888",0,0,7); // Overwrite the text to clear it  
}
...
void showFrequency()
{
    ...
    if (oldFreq == currentFrequency)  return; 

    oldFreq = currentFrequency;
    
    convertToChar(currentFrequency,aux,5);
    ...
    // if ( aux[0] == '0') aux[0] = ' ';
    tft.drawString(aux,0,0,7); // Overwrite the text to clear it
    if ( aux[0] == '0') {
      tft.setTextColor(ST7735_BLACK, ST7735_BLACK); // Set font colour to back to wipe image
      tft.drawChar('8',0,0,7);
    }
