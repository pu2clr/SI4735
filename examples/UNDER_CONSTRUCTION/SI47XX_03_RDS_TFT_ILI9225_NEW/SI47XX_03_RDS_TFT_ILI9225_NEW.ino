/*
  ...
  Under construction... 
  Shrinking de code and improving RDS information
...
void showFrequency() {
  ...
  if (rx.isCurrentTuneFM()) {
    ...
    rx.convertToChar(currentFrequency, bufferDisplay, n, d, '.');
  } else if (currentFrequency < 1000)
    rx.convertToChar(currentFrequency, bufferDisplay, 5, 0, '.', false);
  else rx.convertToChar(currentFrequency, bufferDisplay, 5, 2, '.', false);
...
void showAgcAtt() {
  ...
  rx.convertToChar(agcNdx, aux, 2, 0, false);
  strcat(bufferDisplay, " ");
  strcat(bufferDisplay, aux);
...
void showStep() {
  ...
  rx.convertToChar(tabStep[idxStep], aux, 3, 0, false);
...
char *stationName;
char *rdsTime;
char bufferStatioName[12];
char bufferRdsMsg[63];
char bufferRdsTime[25];

long delayRadioText = millis();
uint8_t idxRadioText = 0;
...
void showRDSMsg() {
  ...
  strncpy(aux, &rdsMsg[idxRadioText], 29);
  aux[29] = '\0';

  printValue(5, 90, bufferRdsMsg, aux, COLOR_GREEN, 6);
  delay(250);
...
void checkRDS() {
  ...
  if (rx.getRdsReceived()) {
    if (rx.getRdsSync() && rx.getRdsSyncFound()) {
      ...
      if (rdsMsg != NULL && strlen(rdsMsg) > 2 && (millis() - delayRadioText) > 1000 )   {
        showRDSMsg();
       delayRadioText =  millis();
      }
...
void showBFO() {
  ...
  aux[0] = (currentBFO >= 0)? '+':'-';
  rx.convertToChar(agcNdx, &aux[1], 4, 0, false);
